...
def validate_execution_role(role_arn: str, session: sagemaker.Session) -> None:
    """Verify the IAM role trust policy allows SageMaker to assume it.

    If the caller lacks permission to read the role, we only emit a warning and
    continue so the user can still attempt the training job.
    """
    ...
    except ClientError as exc:  # pragma: no cover - IAM access varies by environment
        error_code = exc.response.get("Error", {}).get("Code")
        if error_code == "AccessDenied":
            print(
                "Warning: could not validate IAM role trust policy (AccessDenied). "
                "Ensure the role trusts sagemaker.amazonaws.com."
            )
            return
        raise

    statements = role["Role"].get("AssumeRolePolicyDocument", {}).get("Statement", [])
    services: set[str] = set()

    for stmt in statements:
        principal_service = stmt.get("Principal", {}).get("Service")
        if not principal_service:
            continue

        if isinstance(principal_service, list):
            services.update(principal_service)
        else:
            services.add(principal_service)

    if "sagemaker.amazonaws.com" not in services:
        raise ValueError(
            "IAM role trust policy must allow sagemaker.amazonaws.com to assume the role. "
            "Update the role's trust relationship and try again."
        )
...
def submit_training_job(
    ...
    validate_execution_role(role_arn, session)
