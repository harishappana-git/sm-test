...
- The helper script validates that the IAM role trust relationship includes `sagemaker.amazonaws.com` before sending the job. If your user cannot read IAM roles, you will see a warning instead.
- Hyperparameters are passed to the training script with underscores (e.g., `--max_iter`). The training entry point accepts both `--max_iter` and the hyphenated `--max-iter` for local runs.

### Troubleshooting
- **ValidationException: Could not assume role ...**: Update the role trust policy to include the SageMaker service principal (see `docs/aws-setup.md`). After updating, rerun the command.
